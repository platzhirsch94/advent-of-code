...
def get_digits(row, verbose=False, part=1, version=1):
    ...
    if version==1: # Code for 'oneeight' -> 18
        ...
        match_spans = list(chain(*[[(key, match.start(), match.end()) for match in re.finditer(key, row)] 
                       for key in valid_occurences]))
        sorted_match_spans = iter(sorted(match_spans, key=lambda x: x[-1], reverse=True))
...
if __name__ == "__main__":
    ...
    total_sum = sum(get_digits(row, verbose=False, part=PART, version=1) for row in lines_list)
