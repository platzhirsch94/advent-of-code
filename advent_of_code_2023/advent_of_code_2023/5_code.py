...
def merge_ranges(ranges):
    ...
    for start, end in ranges[1:]:
        ...
        if start <= last_end or last_end+1 == start:  # Check for overlap/consecutive numbers
...
def process_seed_ranges(seed_ranges, map_dic):
    for mp in MAPS:
        ...
        mp_a, mp_b, offset = current_map_range[0], current_map_range[1], current_map_range[2]
        ...
        while True:
            ...
            if seed_a >= mp_a:
                ...
                if mp_a <= seed_b <= mp_b:
                    new_seed_ranges.extend([(seed_a,mp_a-1),
                                            (mp_a+offset, seed_b+offset)])
                    try:
                        current_seed_range = next(seed_range_iterator)
                        seed_a, seed_b = current_seed_range[0], current_seed_range[1]
                    except: # All seeds passed
...
def get_lowest_location(lines, part=1):
    ...
    if part == 2:
        seed_ranges= sorted([(it1, it1+it2-1) for it1, it2 in list(zip(seeds[::2], seeds[1::2]))], 
            key=lambda x: x[0])
    ...
    for idx, line in enumerate(lines, start=start_value):
        ...
        dst, src, rng = int(matches.group(1)), int(matches.group(2)), int(matches.group(3))
