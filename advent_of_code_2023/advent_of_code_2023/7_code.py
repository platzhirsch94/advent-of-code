...
TYPE_VALS = ["five", "four", "fh", "three", "two-pair", "pair", "hc"]

CARD_LIST = ["A", "K", "Q", "J", "T", *[str(i) for i in range(9, 1, -1)]]
...
def get_cards(lines, part=1):
    ...
    card_mapper = {
        char: format(idx, "x") for idx, char in enumerate(reversed(CARD_LIST))
    }
    ...
    for line in lines:
        ...
        cards_dic[typ].append(["".join([card_mapper[char] for char in hand]), bid])

    res_ordered_bids = []
    for lst in cards_dic.values():
        res_ordered_bids.extend(
            [*sorted(lst, key=lambda x: int(x[0], 16), reverse=True)]
        )

    weighted_factors = [
        idx * int(tpl[1]) for idx, tpl in enumerate(reversed(res_ordered_bids), start=1)
    ]
